...
int main()
{
	...
	while (true)
	{
		...
		while (!bResetGame)
		{
			...
			//every time a trap is generated, the player gains 5 points
				...
				while (!bHitWall && fDistanceToWall < fDepth)
				{
					...
					else
					{
						...
						if (map[nTestY * nMapWidth + nTestX] == '#')
						{
							...
							//we have four corners to test, so we create two loops to test X coordinates and Y coordinates
			...
			if (map[(int)fPlayerY * nMapWidth + (int)fPlayerX] == 'G')
			{
				nPointTracker += 100;
				bResetGame = true;
				fPlayerX = 14.0f;
